...
test('embedded command dock exposes grouped labelled actions without generic button chrome', () => {
	...
	assert.match(panel, /data-label="\$\{ label \}"/);
	assert.match(panel, /aria-controls="de-panel-buttons"/);
